...
class AuthController {
  ...
  static signup: ExpressFunc = async function (req, res){
    const userRepo = getManager().getRepository(User)
    let email_users= await userRepo.find({email: req.body.email})
    let handle_users = await userRepo.find({handle_name: req.body.handle_name})
  ...
  static checkHandle: ExpressFunc = async function(req, res){
    ...
    if (handle_user){
      let reqUser = await userRepo.findOne(userID)
  ...
  static verifyEmail: ExpressFunc = async function(req, res){
    const userRepo = getManager().getRepository(User)
    let user = await userRepo.findOne(req.params.userID)
  ...
  static cleanup: ExpressFunc = async function(req, res){
    const userRepo = getManager().getRepository(User)
    let users = await userRepo.find({activation_status:'pending'})
